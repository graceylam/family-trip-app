...
type GatewayPayload = Record<string, unknown> & {
  action: "ping" | "getTrip" | "saveTrip" | "upload" | "uploadCover" | "listPhotos" | "getPhoto" | "organize" | "delete" | "placesUsage" | "placesAutocomplete" | "placeDetails";
};
...
export async function uploadTripCoverToDrive(
  tripId: string,
  tripName: string,
  file: File,
  adminSecret: string,
  ...
  const response = await gatewayRequest({
    ...
    tripId,
    tripName,
    name: file.name || "Trip cover.jpg",
    mimeType: file.type || "image/jpeg",
    data: await blobToBase64(file),
    adminSecret,
    previousFileId,
  });
  if (!response.fileId) throw new GoogleDriveError("Google Drive did not return the trip cover.", false);
  return response.fileId;
}
